	...
	actions: {
		...
		async sendMsgCreateGame({ rootGetters }, { value, fee = [], memo = '' }) {
			try {
				...
				const result = await client.TictactoeGame.tx.sendMsgCreateGame({ value, fee: {amount: fee, gas: "200000"}, memo })
				return result
			} catch (e) {
		...
		async sendMsgPlayGame({ rootGetters }, { value, fee = [], memo = '' }) {
			try {
				const client=await initClient(rootGetters)
				const result = await client.TictactoeGame.tx.sendMsgPlayGame({ value, fee: {amount: fee, gas: "200000"}, memo })
				return result
			} catch (e) {
				if (e == MissingWalletError) {
		...
		async MsgCreateGame({ rootGetters }, { value }) {
			try {
				const client=initClient(rootGetters)
				const msg = await client.TictactoeGame.tx.msgCreateGame({value})
				return msg
			} catch (e) {
				if (e == MissingWalletError) {
					throw new Error('TxClient:MsgCreateGame:Init Could not initialize signing client. Wallet is required.')
		...
		async MsgPlayGame({ rootGetters }, { value }) {
			try {
				const client=initClient(rootGetters)
				const msg = await client.TictactoeGame.tx.msgPlayGame({value})
				return msg
			} catch (e) {
				if (e == MissingWalletError) {
					throw new Error('TxClient:MsgPlayGame:Init Could not initialize signing client. Wallet is required.')
